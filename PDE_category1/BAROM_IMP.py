...
class UniversalPDEDataset(Dataset):
    def __init__(self, data_list, dataset_type, train_nt_limit=None): # Added train_nt_limit
        ...
        if dataset_type == 'advection' or dataset_type == 'burgers':
            ...
            self.nx_from_sample = params.get('nx', first_sample['P'].shape[1]) # Use param 'nx' if available
            self.ny_from_sample = params.get('ny', 1) # Use param 'ny' if available, else 1
        else:
...
if __name__ == '__main__':
    ...
    FULL_T_IN_DATAFILE = 2.0  # The T value your .pkl files were generated with
    FULL_NT_IN_DATAFILE = 600 # The nt value your .pkl files were generated with (number of points)
    ...
    # --- Common Parameters ---
    num_controls = 0 # As per your generation commands
    basis_dim = 32
    d_model = 512
    num_heads = 8
    add_error_estimator = False # Set to True if you want to use it
    ...
    run_name = f"{DATASET_TYPE}_b{basis_dim}_d{d_model}_v0"
    #run_name = f"{DATASET_TYPE}_b{basis_dim}_d{d_model}_TESTTEST"
    checkpoint_dir = f"./New_ckpt_2/_checkpoints_{DATASET_TYPE}"
    results_dir = f"./result_all_2/results_{DATASET_TYPE}"
    os.makedirs(checkpoint_dir, exist_ok=True)
    os.makedirs(results_dir, exist_ok=True)
    checkpoint_path = os.path.join(checkpoint_dir, f'rom_{run_name}.pt')
    ...
    if DATASET_TYPE == 'advection':
        ...
        nx = 128; ny = 1; nt = 300; T = 1.0; L = 1.0
        c = 1.0; r = 0.5
        state_keys = ['U']; num_state_vars = 1
        bc_state_dim = 2; bc_state_indices_map = {'U': (0, 1)}
        ...
        nx =128; ny = 1; nt = 300; T = 1; L = 1.0 # Adjusted params
        state_keys = ['rho', 'u']; num_state_vars = 2
        bc_state_dim = 4; bc_state_indices_map = {'rho': (0, 2), 'u': (1, 3)}
        print(f"Loading dataset for {DATASET_TYPE} from {dataset_path}...")
        try:
            with open(dataset_path, 'rb') as f:
                data_list = pickle.load(f)
        ...
        nx = 128; ny = 1; nt = 300; T = 1.0; L = 1.0
        nu = 0.01 / np.pi # Standard parameterization often uses 1/Re
        state_keys = ['U']; num_state_vars = 1
        bc_state_dim = 2; bc_state_indices_map = {'U': (0, 1)}
        print(f"Loading dataset for {DATASET_TYPE} from {dataset_path}...")
        try:
        ...
        nx = 128; ny =1; nt = 300; T = 1.0 # Pseudo-time
        state_keys = ['P']; num_state_vars = 1
        N = nx * ny # Total DoFs for state
        bc_state_dim = 2; bc_state_indices_map = {'P': (0, 1)} # Assuming state indices are 0 and 1
        print(f"Loading dataset for {DATASET_TYPE} from {dataset_path}...")
        try:
            with open(dataset_path, 'rb') as f:
                data_list = pickle.load(f)
        ...
        # Darcy might need different hyperparameters
        basis_dim = 32; d_model = 256; num_heads = 8; batch_size=16; learning_rate = 2e-4
