...
if __name__ == '__main__':
    ...
    try:
        with open(dataset_path, 'rb') as f: data_list_all = pickle.load(f) # Renamed
        ...
        if data_list_all:
            ...
            file_T_check = sample_params_check.get('T', 'N/A')
            file_L_check = sample_params_check.get('L', 'N/A')
            file_nx_check = sample_params_check.get('nx', data_list_all[0][state_keys[0]].shape[1])
            print(f"  Sample 0 from file: nt={file_nt_check}, T={file_T_check}, nx={file_nx_check}, L={file_L_check}")
            if file_nt_check != FULL_NT_IN_DATAFILE:
                print(f"  FATAL: FULL_NT_IN_DATAFILE ({FULL_NT_IN_DATAFILE}) != file's nt ({file_nt_check}). Check script params.")
                exit()
            # Update plot params from file if available and more accurate
            dataset_params_for_plot['T'] = file_T_check if isinstance(file_T_check, (int,float)) else FULL_T_IN_DATAFILE
            dataset_params_for_plot['L'] = file_L_check if isinstance(file_L_check, (int,float)) else 1.0
            dataset_params_for_plot['nx'] = file_nx_check
    ...
    train_data_list_split = data_list_all[:n_train]; val_data_list_split = data_list_all[n_train:] # Renamed
    print(f"Train samples: {len(train_data_list_split)}, Validation samples: {len(val_data_list_split)}")

    # Create training dataset with truncation to TRAIN_NT_FOR_MODEL
    train_dataset = UniversalPDEDataset(train_data_list_split, dataset_type=DATASET_TYPE,
                                        train_nt_limit=TRAIN_NT_FOR_MODEL)
    # Create validation dataset *without* truncation (loads full sequences)
    val_dataset = UniversalPDEDataset(val_data_list_split, dataset_type=DATASET_TYPE,
                                      train_nt_limit=None) # Loads FULL_NT_IN_DATAFILE

    num_workers = 2 # Set to 0 for easier debugging, increase for performance
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True, drop_last=True)
    val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=num_workers) # batch_size=1 for validation

    # Determine FNO input/output channels
    # Input: num_state_vars (from previous time step) + bc_state_dim + num_controls (from current time step)
    # Use bc_state_dim and num_controls from the dataset object after it's initialized
    ...
    fno_output_channels = num_state_vars # Predicts next state for each variable
    ...
    run_name = f"{DATASET_TYPE}_{MODEL_TYPE}_trainT{TRAIN_T_TARGET}_m{fno_modes}_w{fno_width}_{timestamp}"
